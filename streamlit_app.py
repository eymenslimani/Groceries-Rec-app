...
def main():
    ...
    min_confidence = st.sidebar.slider(
        ...
        min_value=0.1, 
        max_value=1.0, 
        value=0.3,  # Lowered from 0.5 to increase chance of finding rules
        step=0.1
    )
    ...
    # Check if rules exist before creating heatmap
    if not current_rules.empty:
        top10_rules = current_rules.nlargest(10, 'confidence')
        
        # Only create heatmap if top10_rules is not empty
        if not top10_rules.empty:
            ...
            st.warning("No rules found with current settings. Try lowering support or confidence thresholds.")
    else:
        st.warning("No association rules found. Try adjusting the support and confidence settings.")
